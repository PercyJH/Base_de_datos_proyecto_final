    ...
    if ( !empty($_POST)) {
        ...
        $id_tramite = $_POST['id_tramite'];
            ...
            $pdo2 = conexion::connect();
                   $sql2 = " SELECT url_pelicula FROM pelicula where id_pro='$id_tramite' ";
       foreach ($pdo2->query($sql2) as $row2) {
               $filename=  $row2['url_pelicula'];
              }
         
        // delete data
        $pdo = conexion::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "DELETE  FROM pelicula  WHERE id_pel = ?";
        $q = $pdo->prepare($sql);
...
<head>
  ...
  <title>Admin | www.yekat.com</title>
